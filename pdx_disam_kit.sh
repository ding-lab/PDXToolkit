...
# set script dir
scriptDir=/gscmnt/gc3021/dinglab/hsun/pdx_kit/pipeline_disam.v1
...
if [[ $pipeline == "dnaFull" ]]; then
	sh $scriptDir/subscripts/DNA.disambiguate.full-step.from_fq.sh -C ${config} -N ${name} -1 ${fq1} -2 ${fq2} -O ${outdir}
fi
...
if [[ $pipeline == "rnaFull" ]]; then
	sh $scriptDir/subscripts/RNA.disambiguate.full-step.from_fq.sh -C ${config} -N ${name} -1 ${fq1} -2 ${fq2} -O ${outdir}
fi
...
if [[ $pipeline == "humanBam" ]]; then
	sh $scriptDir/subscripts/DNA.makeHumanBam.sh -C ${config} -N ${name} -1 ${fq1} -2 ${fq2} -O ${outdir}
fi

# mapping to mouse
if [[ $pipeline == "mouseBam" ]]; then
	sh $scriptDir/subscripts/DNA.makeMouseBam.sh -C ${config} -N ${name} -1 ${fq1} -2 ${fq2} -O ${outdir}
fi
...
if [[ $pipeline == "disambiguate" ]]; then
	sh $scriptDir/subscripts/DNA.disambiguate.sh -C ${config} -N ${name} -D ${dir}
fi
