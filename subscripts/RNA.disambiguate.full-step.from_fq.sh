...
$SAMTOOLS fastq $OUT/$NAME.disam.sortbyname.bam -1 $OUT/$NAME.disam.rna-seq.1.fastq.gz -2 $OUT/$NAME.disam.rna-seq.2.fastq.gz -0 /dev/null -s /dev/null -n -F 0x900

# remove process file for save space
rm -f $OUT/human.sort.bam $OUT/human.Aligned.out.bam $OUT/mouse.sort.bam $OUT/mouse.Aligned.out.bam
